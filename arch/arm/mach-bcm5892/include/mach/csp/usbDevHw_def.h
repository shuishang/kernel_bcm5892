...
#define usbDevHw_ENDPT_CFG_CNT					  8

#define usbDevHw_REG_P			  ((volatile usbDevHw_REG_t *)(MM_IO_BASE_USB_CTLR1))
#define START_USB1_P			  ((volatile usbDevHw_REG_t *)(MM_IO_BASE_USB_CTLR1))
...
static inline void usbDevHw_OpsInit(void)
{
	...
	usbDevHw_OpsFinis();
	REG_WR(usbDevHw_REG_P->devCfg, /*^usbDevHw_REG_DEV_CFG_CSR_PROGRAM_ENABLE */
					/*
					 *^| usbDevHw_REG_DEV_CFG_UTMI_8BIT_ENABLE
					 *^| usbDevHw_REG_DEV_CFG_HALT_STALL_ENABLE
					 *^| usbDevHw_REG_DEV_CFG_SPD_FS_48MHZ		   //BCM5892
					 */

	usbDevHw_REG_DEV_CFG_SPD_FS_48MHZ | usbDevHw_REG_DEV_CFG_REMOTE_WAKEUP_ENABLE);  /*BCM5892 */
...
static inline void usbDevHw_EndptOpsInit(unsigned num, unsigned type, unsigned dirn, unsigned maxPktSize)
{
	...
	/*printk(KERN_ERR "%s:BCM5892:- EP Num= %x:: EP Type= %x ::EP Dirn = %x ::Max Pkt Size=%x\n", __func__,num, type,dirn,maxPktSize); */
