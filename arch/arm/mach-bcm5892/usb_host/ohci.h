...
#if defined(CONFIG_BROADCOM_BCM1103)
#define readl_be(addr)          readl ((__force u32 *)regs)
#define writel_be(val, addr)	writel (val, (__force u32 *)regs)
#endif
...
#elif defined(CONFIG_BROADCOM_BCM1103)

static inline unsigned int
ohci_readl (const struct ohci_hcd *ohci, __hc32 __iomem * regs)
{
	return le32_to_cpu( (__force __le32)readl(regs) );
}

static inline void ohci_writel (const struct ohci_hcd *ohci,
				const unsigned int val, __hc32 __iomem *regs)
{
	writel ( (__force __hc32)cpu_to_le32(val), regs);
}
#else
