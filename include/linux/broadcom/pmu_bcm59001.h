...
#define BCM59001_REG_ENV2        0x02     /* R    environment monitor [3:0] */
   ...
   #define BCM59001_BIT_ENV2_USB_ENTLVL   (1<<5)   /*
                                                    * Main battery working voltage comparator when USB charger is 
                                                    * present. This bit will be set to  
                                                    * above the level specified in USBWVS. 
                                                    */

#define BCM59001_REG_ENV3        0x03     /* R/W  environment monitor [0:0] */
   #define BCM59001_REG_ENV3_TEMPOK (1<<0)   /*
                                              * Temperature OK from the Host. If the Host writes a zero, the 
...
#define BCM59001_REG_HOSTACT     0x04     /* R/W  host action [2:0] (watchdog) */
   #define BCM59001_BIT_HOSTACT_WDT_CLR      (1<<0)   /*
                                                       * Watchdog Clear Bit. Writing a 1 will clear Watchdog timer (bit 
   ...
   #define BCM59001_BIT_HOSTACT_HOSTDICOFF   (1<<2)   /*
                                                       * PMU Turned off by Host. Writing a 1 will turn off the PMU (bit 
...
#define BCM59001_REG_INT1M          0x0A     /* R/W  interrupt mask [6:0] */
   ...
   #define BCM59001_BIT_INT1_PONKEYH   (1<<2)   /*
                                                 * This flag will be set if PONKY is held for a time longer then 
                                                 * the value specified in PONKYHOLD debounce register (PONKYDB) 
                                                 */
   #define BCM59001_BIT_INT1_RTC60S    (1<<3)   /*
                                                 * This bit is set every 60 sec by the Real Time Clock block, This 
                                                 * bit is Cleared after Reading 
                                                 */
   #define BCM59001_BIT_INT1_RTCA1     (1<<4)   /*
                                                 * Alarm 1 generates, if set, by the Real Time Clock. This bit is 
                                                 * Cleared after Reading 
                                                 */
   #define BCM59001_BIT_INT1_RTCADJ    (1<<6)   /*
                                                                                               * When this bit is  
...
#define BCM59001_REG_INT3M           0x0C    /* R/W  interrupt mask [7:0] */
   ...
   #define BCM59001_BIT_INT3_PHFDINS   (1<<3)   /*
                                                 * This bit is set to 1 when Phone Hands Free Input is Inserted. This 
                                                 * corresponds to PHFD level to be below the upper threshold (typically 
                                                 * 1.3V). This bit is Cleared after Reading. 
                                                 */
   #define BCM59001_BIT_INT3_PHFDRM    (1<<4)   /*
                                                 * This bit is set to 1 when Phone Hands Free Input is Removed. This 
                                                 * corresponds to PHFD level to be above the upper threshold (typically 
                                                 * 1.3V). This bit is Cleared after Reading 
                                                 */
   #define BCM59001_BIT_INT3_PHFDPRS   (1<<5)   /*
                                                 * If the PHFD switch, is depressed a low level (i.e. PHFD is 
                                                 * grounded) will be present at the PHFD input, this condition is  
                                                 * indicated in the register INT3, bit-5 (PHFDPRS) set to 1 
                                                 */
   #define BCM59001_BIT_INT3_LOWBAT    (1<<6)   /*
                                                 * This bit is set to 1 when the Main Battery voltage VMBAT is below 
...
#define BCM59001_REG_CSRCTRL9        0x29    /*
                                              * R/W  core switching reg control [2:0] 
                                         * bit layouts for PC2,PC1 status are: 0,0[1:0] 0,1[3:2] 1,0[5:4] 1,1[7:6] 
                                         */
...
#define BCM59001_REG_IOSRCTRL9       0x33    /*
                                              * R/W  io switching reg control [2:0] 
                                         * bit layouts for PC2,PC1 status are: 0,0[1:0] 0,1[3:2] 1,0[5:4] 1,1[7:6] 
                                         */
...
#define BCM59001_REG_MBCCTRL2        0x37    /* R/W  main battery charger control [7:0] */
   ...
   #define BCM59001_BIT_MBCCTRL2_MBCHOSTEN  (0x01<<6)  /*
                                                        * MBC enabled from Host (overwrite the 4 enable 
...
#define BCM59001_REG_FGCTRL3_FGCAL   (1<<1)  /*
                                              * Host to calibrate the fuel gauge. Set to 1 to start 
...
#define BCM59001_REG_FGCTRL3_FGSNAPSHOT (1<<3)  /*
                                                 * Fuel gauge snapshot. Set to 1 to latch the 
