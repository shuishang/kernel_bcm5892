...
int ubi_io_mark_bad(const struct ubi_device *ubi, int pnum)
{
	...
	/* 
	 * For NOP=1 MLC devices, the bad block marker cannot be written if 
	 * other bytes later in the oob are already written,such as ecc.
	 * So try to erase the block first before writing the oob to 
	 * support NOP=1 devices.
	 */
	err = do_sync_erase((struct ubi_device *)ubi, pnum);
	if (err)
		ubi_err("cannot erase PEB %d prior to marking it bad, error %d", pnum, err);
