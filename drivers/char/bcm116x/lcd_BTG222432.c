...
void
lcd_ResetStartAddr(LCD_dev_info_t *dev)
{
    uint32_t left = 0 + dev->col_offset;
    /*uint32_t right = dev->width + dev->col_offset - 1; */
    uint32_t top = 0 + dev->row_offset;
    /*uint32_t bottom = dev->height + dev->row_offset - 1; */

#if 0
    WRITE_LCD_CMD(LCD_CMD(REN_WINDOW_HORIZONTAL_RAM_ADDRESS_START));
    WRITE_LCD_PARAM(LCD_DATA(left));

    WRITE_LCD_CMD(LCD_CMD(REN_WINDOW_HORIZONTAL_RAM_ADDRESS_END));
    WRITE_LCD_PARAM(LCD_DATA(right));

    WRITE_LCD_CMD(LCD_CMD(REN_WINDOW_VERTICAL_RAM_ADDRESS_START));
    WRITE_LCD_PARAM(LCD_DATA(top));

    WRITE_LCD_CMD(LCD_CMD(REN_WINDOW_VERTICAL_RAM_ADDRESS_END));
    WRITE_LCD_PARAM(LCD_DATA(bottom));
#endif
...
static int
lcd_send_cmd_sequence(Lcd_init_t *init)
{
    int i;
