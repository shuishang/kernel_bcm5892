...
#include <linux/broadcom/bcm_major.h>        /* For BCM_VCP_MAJOR */
...
static int     vcp_ioctl( struct inode  *inode, struct file *file, unsigned int cmd, unsigned long arg );
...
static int __init vcp_init( void )
{
   ...
   err = register_chrdev( BCM_VCP_MAJOR, "vcp", &gfops );
   if ( err )
   {
      printk( "%s: failed to register character device major=%d\n", __FUNCTION__, BCM_VCP_MAJOR );
      goto failed_cleanup;
   }
	...
	vcp_dev = device_create( vcp_class, NULL, MKDEV( BCM_VCP_MAJOR, 0 ), NULL, "vcp");
	if ( IS_ERR( vcp_dev ))
	{
...
err_unregister_chrdev:
	unregister_chrdev( BCM_VCP_MAJOR, "vcp" );
failed_cleanup:
...
static void __exit vcp_exit( void )
{
   ...
   unregister_chrdev( BCM_VCP_MAJOR, "vcp" );
	device_destroy( vcp_class, MKDEV( BCM_VCP_MAJOR, 0 ));
	class_destroy( vcp_class );
}
