...
#define SND_HALAUDIO_MAX_PCMDEVS             4
...
static const char *pcm_dev_list[] = { "speakerphone", "handset", "headset", "lineout", "linein", "bt-headset" };
...
static struct snd_kcontrol_new snd_halaudio_sw_gain_controls[] =
{
...
)
{
   ...
   if ( err )
   {
      snd_printk( KERN_ERR "%s: failed to create mixer port %s\n", __FUNCTION__, portname );
      return err;
   }
   ...
   if ( err )
   {
      snd_printk( KERN_ERR "%s: failed to find codec channel %s\n", __FUNCTION__, sp->pcmcfg.info.codec_name );
      return err;
   }
...
)
{
   ...
   if ( err )
   {
      ...
      */
      ...
      if ( err )
      {
         snd_printk( KERN_ERR "%s: failed to set to rate=%i for codec %i [%s]\n",
   ...
   if ( err )
   {
      snd_printk( KERN_ERR "%s: failed to query hw port info err=%i\n", __FUNCTION__, err );
      return -EINVAL;
   }
   ...
   {
      snd_printk( KERN_ERR "%s: buffer size of %i bytes is less than minimum expected at %i bytes\n",
   ...
   if ( err )
   {
      snd_printk( KERN_ERR "%s: failed to set %s amxr port sample rate=%i\n",
            __FUNCTION__,
   ...
   if ( err )
   {
      snd_printk( KERN_ERR "%s: failed to set %s amxr port channels=%i\n",
            __FUNCTION__,
            subsp->dir == SNDRV_PCM_STREAM_PLAYBACK ? "playback" : "capture",
            runtime->channels );
      return err;
   ...
   if ( sp->pcmcfg.has_aux )
   {
      ...
      {
         snd_printk( KERN_ERR "%s: unsupported config for combo device channels=%i\n",
   ...
   else
   {
      ...
      {
         snd_printk( KERN_ERR "%s: unsupported config src_chans=%i dst_chans=%i\n",
               __FUNCTION__, src_chans, dst_chans );
         return -EINVAL;
      }

      desc = AMXR_CREATE_DESC( cnx_type, src_chans, 0, dst_chans, 0 );
   }
   ...
   if ( err )
   {
      ...
      snd_printk( KERN_ERR "%s: failed to connect %s -> %s\n",
   ...
   if ( err )
   {
      snd_printk( KERN_ERR "%s: failed to enable full audio power err=%i\n",
            __FUNCTION__, err );
      return err;
   }
...
)
{
   ...
   for ( i = 0; i < numdevs && pcmid < SND_HALAUDIO_MAX_PCMDEVS; i++ )
   {
...
#ifdef CONFIG_BCM_BSC
      err = bsc_query( pcm_dev_list[i], &pcmcfg, sizeof(pcmcfg) );
      if ( err )
      {
         /* Device not found */
         continue;
      ...
      err = snd_pcm_new( card, "HAL Audio", pcmid,
            SND_HALAUDIO_MAX_SUBS, SND_HALAUDIO_MAX_SUBS, &pcm );
      if ( err )
      {
         goto cleanup_and_exit;
      }
      ...
      streamp->pcmid       = pcmid;
      memcpy( &streamp->pcmcfg, &pcmcfg, sizeof(streamp->pcmcfg) );

      pcm->private_data    = streamp;
      pcm->private_free    = snd_halaudio_pcm_free;
      pcm->info_flags      = 0;
      sprintf( pcm->name, "%s", pcmcfg.name );
      ...
      /* FIXME - should sanity check control before adding them */
      ...
      /* Add software gains */
      err = add_controls( card, streamp, snd_halaudio_sw_gain_controls, ARRAY_SIZE( snd_halaudio_sw_gain_controls ));
      if ( err )
      {
         goto cleanup_and_exit;
      }
   ...
   if ( err )
   {
      snd_printk( KERN_ERR "%s: failed to register sound card\n", __FUNCTION__ );
      goto cleanup_and_exit;
   }
