...
static int md5_spu_init(struct shash_desc *desc)
{
	struct md5_spu_ctx *mctx = shash_desc_ctx(desc);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
static int md5_spu_update(struct shash_desc *desc, const u8 *data, unsigned int len)
{
	struct md5_spu_ctx *mctx = shash_desc_ctx(desc);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
static int md5_spu_final(struct shash_desc *desc, u8 *out)
{
	struct md5_spu_ctx *mctx = shash_desc_ctx(desc);
#else
...
static struct shash_alg alg = {
	...
	.init           =       md5_spu_init,
	.update         =       md5_spu_update,
	.final          =       md5_spu_final,
	.descsize       =       sizeof(struct md5_spu_ctx),
	.base           =       {
		.cra_name       =       "md5",
