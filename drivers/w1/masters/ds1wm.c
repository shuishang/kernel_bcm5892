...
#ifdef CONFIG_ARCH_BCM5892
static DEFINE_MUTEX(ds1wm_mutex);

static inline void bcm5892_d1w_write_reg(struct ds1wm_data *ds1wm_data, u32 reg,
                    u8 val)
{
    writel(val, ds1wm_data->map + D1W_DIN);
    writel(reg, ds1wm_data->map + D1W_ADDR);
    writel(0x3, ds1wm_data->map + D1W_CTL);
}

static inline u8 bcm5892_d1w_read_reg(struct ds1wm_data *ds1wm_data, u32 reg)
{
    writel(reg, ds1wm_data->map + D1W_ADDR);
    writel(0x2, ds1wm_data->map + D1W_CTL);
    return readb(ds1wm_data->map + D1W_DOUT);
...
{
#ifdef CONFIG_ARCH_BCM5892
    bcm5892_d1w_write_reg(ds1wm_data, reg, val);
#else
...
{
#ifdef CONFIG_ARCH_BCM5892
    return bcm5892_d1w_read_reg(ds1wm_data, reg);
#else
...
#ifdef CONFIG_ARCH_BCM5892
    mutex_lock(&ds1wm_mutex);
#endif
    while (ds1wm_data->reset_complete)
        msleep(1);
...
#ifdef CONFIG_ARCH_BCM5892
    /* set the CLK_EN bit */
    ds1wm_write_register(ds1wm_data, DS1WM_CLKDIV , divisor | 0x80);
#else
