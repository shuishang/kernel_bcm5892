/*
 *  linux/drivers/serial/broadcom-bcm11211_serial.c
 *
 *  Driver for Broadcom bcm11211 serial ports
 *
 ...
 *
 * 25-Aug-2010  SP   Added BT power management initialization function call.
 *
 */

#if defined(CONFIG_SERIAL_BROADCOM_BCM11211_CONSOLE) && defined(CONFIG_MAGIC_SYSRQ)
...
#define BCM11211_UART_MOD_DESCRIPTION	"BCM11211 UART Driver"
#define BCM11211_UART_MOD_VERSION		2.0

#define DEBUG_IO                0   // Debug IO TX/RX data
#define DEBUG_RTS_DTR_LBE_OUT1  0   // Turn on/off pins RTS, DTR, LBE, OUT1

// this is to resolve the problem in which first time read the GPS port,
// the FIFO RX interrupt status register cannot be clear even though RX FIFO is empty
#define GPS_UART_WORKAROUND			1

#ifdef CONFIG_ARCH_BCMRING
#define UART_NR			2
#elif defined(CONFIG_ARCH_BCM476X)
#define UART_NR			4
#else
#define UART_NR			14
#endif
...
//#if defined(CONFIG_ARCH_BCMRING) || defined(CONFIG_ARCH_BCM476X)
 ...
 * not used separately.
 */
struct bcm11211_uart_suspend_data
{
	uint16_t	uart_ibrd;
	uint16_t	uart_fbrd;
	uint8_t		uart_lcr;
...
struct uart_amba_port {
	...
	struct bcm11211_uart_suspend_data str_state;	/* register data to be preserved during suspend to RAM */
};

/* Defines */
#define PFX "amba-pl011: "
#if defined(CONFIG_BCMRING_GPS) && defined(CONFIG_BCMRING_GPS_UART)
#define GPS_PORT  CONFIG_BCMRING_GPS_UART
#else
#define GPS_PORT 999  // this is tricky so that it would never called this
#endif
...
static void bcm11211_uart_tx_chars(struct uart_amba_port *uap);
...
static void bcm11211_uart_stop_tx(struct uart_port *port)
{
...
static void bcm11211_uart_start_tx(struct uart_port *port)
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;
...
static void bcm11211_uart_stop_rx(struct uart_port *port)
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;
...
static void bcm11211_uart_enable_ms(struct uart_port *port)
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;
...
static void bcm11211_uart_rx_chars(struct uart_amba_port *uap)
{
...
static void bcm11211_uart_tx_chars(struct uart_amba_port *uap)
{
	...
	if (uart_circ_empty(xmit) || uart_tx_stopped(&uap->port)) {
		bcm11211_uart_stop_tx(&uap->port);
		return;
	}
...
static void bcm11211_uart_modem_status(struct uart_amba_port *uap)
{
...
static irqreturn_t bcm11211_uart_int(int irq, void *dev_id)
{
	...
	if (status) { 
		do {
#if GPS_UART_WORKAROUND  
			// this may not be best solution as it would cause the problem on race condition
			// see GPS_UART_WORKAROUND define above regarding the issue
			volatile unsigned int flag_status = readw(uap->port.membase + UART01x_FR);
			if ((port->line == GPS_PORT) && (status & UART011_RXIS) && (flag_status & UART01x_FR_RXFE) && (flag_status != UART01x_FR_BUSY))
			{
				printk("GPS Port interrupt status=0x%x, flag_status=0x%x\n", status, flag_status);
				writew(status & ~UART011_TXIS, uap->port.membase + UART011_ICR);
			}
			else
			{
			...
			{
				bcm11211_uart_rx_chars(uap);
			}
			if (status & (UART011_DSRMIS|UART011_DCDMIS|
				      UART011_CTSMIS|UART011_RIMIS))
			{  
				bcm11211_uart_modem_status(uap);
			} 
...
static void bcm11211_uart_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;
	unsigned int cr;
...
static void bcm11211_uart_break_ctl(struct uart_port *port, int break_state)
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;
	unsigned long flags;
	unsigned int lcr_h;
...
static int bcm11211_uart_startup(struct uart_port *port)
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;
	unsigned int cr;
	int retval;

	dbg("bcm11211_uart_startup: port=%p (%08lx,%p), line=%u\n",
	 ...
	 * Allocate the IRQ
	 */
	retval = request_irq(uap->port.irq, bcm11211_uart_int, 0, "uart-pl011", uap);
	if (retval) {
...
//	cr = UART01x_CR_UARTEN | UART011_CR_RXE | UART011_CR_TXE;
	...
	dbg("bcm11211_uart_startup ok, IM=%08lx CR=%08lx\n",uap->im,cr);
...
static void bcm11211_uart_shutdown(struct uart_port *port)
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;
	unsigned long val;

	val = readw(uap->port.membase + UART011_IMSC);
	dbg("bcm11211_uart_shutdown: port=%p (%08lx,%p),IMSC=%08lx im=%08lx line=%u\n",
...
static void
bcm11211_uart_set_termios(struct uart_port *port, struct ktermios *termios,
		     struct ktermios *old)
{
	...
	dbg("bcm11211_uart_set_termios: port=%d\n",port->line);
	...
	if (UART_ENABLE_MS(port, termios->c_cflag))
		bcm11211_uart_enable_ms(port);

	/* first, disable everything */
	old_cr = readw(port->membase + UART011_CR);
	old_cr &= ~UART011_CR_TXE;

	dbg("bcm11211_uart_set_termios: c_cflag=0x%08x, CRTSCTS=0x%08x old_cr=0x%08x\n",termios->c_cflag,termios->c_cflag & CRTSCTS,old_cr);
...
//       old_cr |= UART011_CR_TXE | UART011_CR_RXE | UART01x_CR_UARTEN;
	   ...
	   if ( termios->c_cflag & CRTSCTS )
	   {
		  ...
		  dbg("bcm11211_uart_set_termios.1: c_oflag=0x%08x, TIOCM_RTS=0x%08x old_cr=0x%08x\n",termios->c_oflag,termios->c_oflag & DBG_TIOCM_RTS,old_cr);
          ...
          dbg("bcm11211_uart_set_termios.2: c_oflag=0x%08x, TIOCM_DTR=0x%08x old_cr=0x%08x\n",termios->c_oflag,termios->c_oflag & DBG_TIOCM_DTR,old_cr);
          ...
          dbg("bcm11211_uart_set_termios.3: c_oflag=0x%08x, CR_LBE=0x%08x old_cr=0x%08x\n",termios->c_oflag,termios->c_oflag & DBG_CR_LBE,old_cr);
	   }
       ...
       dbg("bcm11211_uart_set_termios.4: c_oflag=0x%08x, TIOCM_OUT1=0x%08x old_cr=0x%08x\n",termios->c_oflag,termios->c_oflag & DBG_TIOCM_OUT1,old_cr);
	...
	dbg("bcm11211_uart_set_termios: uart: LCRH = 0x%08x, CR = 0x%08x, IFLS = 0x%08x, IMSC = 0x%08x\n",
...
static const char *bcm11211_uart_type(struct uart_port *port)
{
...
static struct uart_ops amba_bcm11211_uart_pops = {
	.tx_empty	= pl01x_tx_empty,
	.set_mctrl	= bcm11211_uart_set_mctrl,
	.get_mctrl	= pl01x_get_mctrl,
	.stop_tx	= bcm11211_uart_stop_tx,
	.start_tx	= bcm11211_uart_start_tx,
	.stop_rx	= bcm11211_uart_stop_rx,
	.enable_ms	= bcm11211_uart_enable_ms,
	.break_ctl	= bcm11211_uart_break_ctl,
	.startup	= bcm11211_uart_startup,
	.shutdown	= bcm11211_uart_shutdown,
	.set_termios	= bcm11211_uart_set_termios,
	.type		= bcm11211_uart_type,
	.release_port	= pl010_release_port,
	.request_port	= pl010_request_port,
	.config_port	= pl010_config_port,
	.verify_port	= pl010_verify_port,
#ifdef CONFIG_RFKILL_PM
	.ioctl          = serial11211_ioctl,
#endif
#ifdef CONFIG_CONSOLE_POLL
 ...
 */
int bcm11211_uart_serial_get_port_info(int line,char *szBuf)
{
    register char *pos;
    ...
    if ( port )
    {
		...
		{ 
           pos += sprintf(pos,"bcm11211_uart_serial_get_port_info: port->line=%d\n",port->line);
...
#ifdef CONFIG_SERIAL_BROADCOM_BCM11211_CONSOLE

static void bcm11211_uart_console_putchar(struct uart_port *port, int ch)
{
	struct uart_amba_port *uap = (struct uart_amba_port *)port;
...
static void
bcm11211_uart_console_write(struct console *co, const char *s, unsigned int count)
{
	...
	uart_console_write(&uap->port, s, count, bcm11211_uart_console_putchar);
...
static void __init
bcm11211_uart_console_get_options(struct uart_amba_port *uap, int *baud,
			     int *parity, int *bits)
{
...
static int __init bcm11211_uart_console_setup(struct console *co, char *options)
{
	...
	dbg("bcm11211_uart_console_setup: co=%p (%d), %s\n",co, co->index, options);
	...
	dbg("bcm11211_uart_console_setup: port=%p (%d)\n", &uap->port, co->index);

	if (options)
		uart_parse_options(options, &baud, &parity, &bits, &flow);
	else
		bcm11211_uart_console_get_options(uap, &baud, &parity, &bits);

	dbg("bcm11211_uart_console_setup: baud %d\n", baud);
...
//#if defined(CONFIG_ARCH_BCMRING) || defined(CONFIG_ARCH_BCM476X)
	...
	.name		= "ttyAMA",
#endif
	.write		= bcm11211_uart_console_write,
	.device		= uart_console_device,
	.setup		= bcm11211_uart_console_setup,
...
static int bcm11211_uart_probe(struct amba_device *dev, void *id)
{
	struct uart_amba_port *uap;
	void __iomem *base;
	int i, ret;
	...
	uap->port.ops = &amba_bcm11211_uart_pops;
	...
	{ 
	  struct uart_port *port = &uap->port;
	  dbg("bcm11211_uart_probe: port=%p, id=%p\n", port, id);

	  dbg("resource %p (%lx..%lx)\n", &dev->res, dev->res.start, dev->res.end);

	  dbg("bcm11211_uart_probe: line=%u map=%08x, mem=%08x, irq=%d, clock=%ld\n",
...
static int bcm11211_uart_remove(struct amba_device *dev)
{
 ...
 */
int bcm11211_uart_suspend(struct amba_device *dev, pm_message_t pm)
{
	struct uart_amba_port *uap = amba_get_drvdata(dev);
...
} /* bcm11211_uart_suspend */

/* @brief bcm11211_uart_resume
 *
 ...
 */
int	bcm11211_uart_resume(struct amba_device *dev)
{
	struct uart_amba_port *uap = amba_get_drvdata(dev);

	dbg("Resume processed for %s.\n", pdev->name); 
	...
	return 0;
} /* bcm11211_uart_resume */

static struct amba_id bcm11211_uart_ids[] __initdata = {
	{
		.id	= 0x00041011,
		.mask	= 0x000fffff,
	},
...
static struct amba_driver bcm11211_uart_driver = {
	.drv = {
		.name	= "uart-bcm11211",
	},
	.id_table	= bcm11211_uart_ids,
	.probe		= bcm11211_uart_probe,
	.remove		= bcm11211_uart_remove,
#ifdef CONFIG_RFKILL_PM
	.suspend    = bcm11211_uart_suspend,
	.resume     = bcm11211_uart_resume,
#endif
};

static int __init bcm11211_uart_init(void)
{
	int ret;
	printk(KERN_INFO "Serial: BCM11211 UART driver with GPS port=%d\n", GPS_PORT);

	ret = uart_register_driver(&amba_reg);
	if (ret == 0) {
		ret = amba_driver_register(&bcm11211_uart_driver);
		if (ret)
...
static void __exit bcm11211_uart_exit(void)
{
	amba_driver_unregister(&bcm11211_uart_driver);
	uart_unregister_driver(&amba_reg);
}

module_init(bcm11211_uart_init);
module_exit(bcm11211_uart_exit);

MODULE_AUTHOR("Broadcom/ARM Ltd/Deep Blue Solutions Ltd");
MODULE_VERSION(BCM11211_UART_MOD_VERSION);
MODULE_DESCRIPTION(BCM11211_UART_MOD_DESCRIPTION);
MODULE_LICENSE("GPL");
