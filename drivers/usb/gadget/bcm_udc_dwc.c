...
#ifdef CONFIG_ARCH_BCM5892
#define BCM_UDC_IN_TX_MAX        (0x3FFF & ~(64-1))

#define OVIC0_IRQ_START            32
#define IRQ_OUSBH1            (OVIC0_IRQ_START + 27)
#define BCM_UDC_IRQ            IRQ_OUSBH1
#else
...
#include <linux/broadcom/usb_udc.h>
...
#ifdef CONFIG_ARCH_BCM5892
#include <mach/csp/usbDevHw.h>
#include <mach/csp/usbPhyHw.h>
#else
...
#ifdef CONFIG_ARCH_BCM5892
static void bcm5892_usb_pullup(void);
static void bcm5892_usb_pulldown(void);
#endif
/* ---- Private Variables ------------------------------------------------ */
...
#ifdef CONFIG_ARCH_BCM5892
extern int bcm5892_pm_udc_wakeup(void);
#endif
#else
...
#define USB_SRAM_DMA_START              0x38000000
#define USB_SRAM_DMA_END                0x38500000   /* 5 MB Space */

static struct resource bcm5892_udc_resource[] =
{
        [0] =
                {
        ...
        [3] =
                {
                .flags  = IORESOURCE_MEM,
                .start  = USB_SRAM_DMA_START,
                .end    = USB_SRAM_DMA_END,
                },
};

static void bcm5892_udc_release (struct device *dev) {}

static struct platform_device bcm5892_udc_pdev = {

        .name   = BCM_UDC_NAME,
        .id     = 0,
        .dev    = {
                        .coherent_dma_mask = DMA_32BIT_MASK,
                        .dma_mask = &bcm5892_udc_pdev.dev.coherent_dma_mask,
                        .release = bcm5892_udc_release,
                },
        .resource =bcm5892_udc_resource,
        .num_resources=ARRAY_SIZE(bcm5892_udc_resource),
 ...
 */
    ...
    {
        err = platform_device_register( &bcm5892_udc_pdev );
        if (err)
        {
...
{
#ifdef CONFIG_ARCH_BCM5892
    platform_device_unregister( &bcm5892_udc_pdev );
#endif
    platform_driver_unregister( &bcm_udc_PlatformDriver );
}
...
int usb_gadget_register_driver( struct usb_gadget_driver *gadget_driver )
{
    ...
    BCM_DEBUG_TRACE( "enter\n" );

    if ( bcmUdcP == NULL )
    {
        BCM_KERROR( "UDC driver not initialized\n" );
        BCM_DEBUG_TRACE( "exit\n" );

        return( -ENODEV );
    }

    if ( !gadget_driver || !gadget_driver->bind || !gadget_driver->setup || (gadget_driver->speed < USB_SPEED_FULL) )
    {
    ...
    BCM_DEBUG_DEV( "%s gadget_driver->bind()\n", gadget_driver->driver.name );
...
{    
    ...
    if ( bcmUdcP && bcmUdcP->gadget_driver && !bcm5892_pm_udc_wakeup())
    {
...
#ifdef CONFIG_ARCH_BCMRING
    if (gVbusDetectGpio >= 0) {
       bcm_udc_gadgetDevOps.wakeup = GadgetDevWakeup;
       bcm_udc_gadgetDevOps.set_selfpowered = GadgetDevSetSelfpowered;
       bcm_udc_gadgetDevOps.vbus_session = GadgetDevVbusSession;
    }
#endif
...
int GadgetEpEnable( struct usb_ep *usb_ep, const struct usb_endpoint_descriptor *desc )
{
    ...
    maxPktSize = le16_to_cpu (desc->wMaxPacketSize) & USB_ENDPOINT_SIZE_MAX_MASK;
...
{
    KNLLOG( "%s virt=0x%p phys=0x%p: 0x%08x 0x%08x 0x%08x", label, virt, phys, virt->status, virt->reserved, virt->bufAddr );
}
...
void DmaDataFinis( BCM_UDC_EP_t *udcEpP )
{
    ...
    BCM_DEBUG_DMA( "%s: buf=0x%x dma=0x%x actual=%u\n",
                    udcEpP->usb_ep.name, udcEpReqP->usb_req.buf, udcEpReqP->usb_req.dma, udcEpReqP->usb_req.actual);
...
void DmaDataAddReady( BCM_UDC_EP_t *udcEpP )
{
    ...
    if ( (udcEpP->dirn == USB_DIR_OUT) && (udcEpP->type == USB_ENDPOINT_XFER_BULK) )
        DmaDumpEp( udcEpP );
...
static void UdcOpsInit( BCM_UDC_t *udcP )
{
...
#ifdef CONFIG_ARCH_BCM5892
    /* On the 5892, endpoints 1-4 are bi-directional bulk endpoints. */
    ...
    /* On the 5892, endpoints 5-8 are Interrupt-IN endpoints */
...
irqreturn_t IrqUdc(int irq, void *context)
{
    ...
    BCM_DEBUG_IRQ( "enter: mask: dev=0x%x ep=0x%x  status: dev=0x%x ep=0x%x\n",
                    usbDevHw_REG_P->devIntrMask, usbDevHw_REG_P->eptIntrMask,
                    usbDevHw_REG_P->devIntrStatus, usbDevHw_REG_P->eptIntrStatus );
...
void IrqEpInStatusCheck( BCM_UDC_EP_t *udcEpP )
{
    ...
    if ( status & usbDevHw_ENDPT_STATUS_IN_DMA_DONE )
    {
        ...
        {
            /* No XFER_DONE interrupt on the BCM5892 */
            ReqXferProcess( udcEpP );
        }
#else
...
#ifdef CONFIG_ARCH_BCM5892
/***************************************************************************
 * BCM5892 support code
 ***************************************************************************/
void bcm5892_usb_pullup(void)
{
        uint32_t reg_val = 0;

        BCM_KERROR( "%s\n", __func__ );
        /* (i) Choose output mode for [11] : Write a 1 to bit 11 of this register 0xcd808 GIO1_R_GRPF1_OUT_ENABLE_MEMADDR */

        reg_val |= 1 << 11;
        REG_WR(usbDevPull_REG_P->GPIO1_enable, reg_val);
        reg_val = __raw_readl(MM_IO_BASE_GIO1_R_GRPF1_DATA_OUT_MEMADDR);

        /* ii) Drive out a 1 or a 0 on bit 11 : by writing to this register 0xcd804 GIO1_R_GRPF1_DATA_OUT_MEMADDR */

        /* reverse logic on checkout board */

        reg_val = __raw_readl(MM_IO_BASE_GIO1_R_GRPF1_DATA_OUT_MEMADDR);
        reg_val &= ~(1 << 11);
        __raw_writel(reg_val, MM_IO_BASE_GIO1_R_GRPF1_DATA_OUT_MEMADDR);

        return;
}

void bcm5892_usb_pulldown(void)
{
        uint32_t reg_val;
        BCM_KERROR( "%s\n", __func__ );
        /* (i) Choose output mode for [11]  : Write a 1 to bit 11 of this register 0xcd808 GIO1_R_GRPF1_OUT_ENABLE_MEMADDR */
        reg_val = __raw_readl(MM_IO_BASE_GIO1_R_GRPF1_OUT_ENABLE_MEMADDR);
        reg_val |= 1 << 11;
        __raw_writel(reg_val,MM_IO_BASE_GIO1_R_GRPF1_OUT_ENABLE_MEMADDR);
