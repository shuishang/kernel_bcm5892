...
found:
	...
	if (in) {
		...
		dev_dbg (&udev->dev, "bulk in=%d out=%d\n", 
			in->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK, 
			out->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK );
	}
	if (iso_in) {
		...
		dev->out_iso_pipe = usb_sndisocpipe (udev,
				iso_out->desc.bEndpointAddress
					& USB_ENDPOINT_NUMBER_MASK);
		dev_dbg (&udev->dev, "iso in=%d out=%d\n", 
...
)
{
	...
	while (retval == 0 && iterations-- > 0) {
		...
		if (retval == 0 && usb_pipein (urb->pipe))
			...
			dev_dbg (&udev->dev, "%s completion failed, status %d\n", label, retval);
		...
		int pipe, struct usb_endpoint_descriptor *desc)
{
	...
	dev_info(&dev->intf->dev,
		"... iso period %d %sframes, wMaxPacket %04x, length %04x\n",
		1 << (desc->bInterval - 1),
		(udev->speed == USB_SPEED_HIGH) ? "micro" : "",
		le16_to_cpu(desc->wMaxPacketSize), param->length);
	...
	if (status != 0)
		...
		status = -EIO;
	dev_dbg (&dev->intf->dev, "complete: status=%d\n", status);
	return status;

fail:
	dev_dbg (&dev->intf->dev, "fail: status=%d\n", status);
	for (i = 0; i < param->sglen; i++) {
		if (urbs [i])
			simple_free_urb (urbs [i]);
	}
