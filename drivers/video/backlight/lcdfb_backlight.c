...
static int __init lcdfb_backlight_probe( struct platform_device *pdev )
{
    struct backlight_device *bl_dev;

    lcd_backlight_init( pdev->dev.platform_data );

    bl_dev = backlight_device_register( "lcdfb-bl", &pdev->dev, NULL, &lcdfb_backlight_ops );
    if ( IS_ERR( bl_dev ))
    {
...
static int lcdfb_backlight_remove( struct platform_device *pdev )
{
